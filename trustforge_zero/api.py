...
app = FastAPI(
    ...
    version="0.6.0",
    description="Evidence-first autonomous immune mesh with fail-closed runtime recovery for synthetic AI agent fleets.",
)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def _live_run(
    ...
    yield await push(
        ...
        {"target": target, "sandbox": True, "recovery_mode": recovery_mode},
    )
    yield await push(
    ...
    # Runtime resilience proof. The default demo injects a synthetic forensic
    # timeout, then proves isolation, checkpoint continuity, reassignment, and
    # replay before certification can continue.
    ...
    if inject_failure:
        yield await push(
            ...
            "recover",
            "DETECTED",
            "Governor detected a synthetic forensic-agent timeout before remediation was trusted.",
            {"agent": recovery.agent, "failure_mode": recovery.failure_mode},
        )
        yield await push(
            "AGENT_ISOLATED",
            "governor",
            "recover",
            "ISOLATED" if recovery.isolated else "FAILED",
            "Governor isolated the failed execution slot to prevent contaminated state propagation.",
            {"agent": recovery.agent, "isolated": recovery.isolated},
        )
        yield await push(
            "CHECKPOINT_RESTORED",
            "governor",
            "recover",
            "RESTORED" if recovery.checkpoint_id else "FAILED",
            "Governor restored the last trusted pre-diagnosis checkpoint.",
        ...
        yield await push(
            ...
            "governor",
            "recover",
            "REASSIGNED" if recovery.reassigned_to else "FAILED",
            "Governor reassigned forensic analysis to a clean recovery slot.",
            {"reassigned_to": recovery.reassigned_to, "retries": recovery.retries},
        )
        yield await push(
            "RECOVERY_REPLAY_VERIFIED",
            "forensic_agent",
            "recover",
            "VERIFIED" if recovery.replay_verified else "FAILED",
            "Recovered forensic trajectory was replayed against the trusted checkpoint and verified." if recovery.replay_verified else "Recovery replay could not be verified; certification gate is closed.",
            recovery.to_dict(),
        )
    else:
        yield await push(
            "RECOVERY_BASELINE_HEALTHY",
            "governor",
            "recover",
            "VERIFIED",
            "No runtime failure was injected; recovery control plane remained certification-safe.",
            recovery.to_dict(),
        )
    ...
    yield await push(
        ...
        "repair",
        "HARDENED",
        "Defense applied the minimum least-privilege repair bundle in the synthetic sandbox.",
        patch,
    )
    yield await push(
        "MEMORY_GUARD_ARMED",
        ...
        "repair",
        "HARDENED",
        "Memory Guard provenance-gated persistent writes and promoted verified failures into regression requirements.",
        {"provenance_required": True, "regression_memory": True},
    )
    ...
    for result in after:
        ...
        yield await push(
            ...
            f"Hardened replay {result['test']} produced {decision}.",
            result,
        )
    ...
    yield await push(
        ...
        "Provenance verified replay comparability, claim-to-event lineage, runtime recovery evidence, and continuous evidence integrity." if chain_before_attestation else "Provenance rejected the evidence lineage.",
        {
    ...
    judge_allowed = bool(chain_before_attestation and recovery_gate_open)
    ...
    yield await push(
        ...
        "certify",
        judge_status,
        f"Judge verified {passport['tests_passed']}/{passport['tests_total']} controls, provenance, and runtime recovery." if judge_allowed else "Judge blocked certification because runtime recovery or provenance evidence did not satisfy the fail-closed gate.",
        {**passport, "runtime_recovery_verified": recovery_gate_open},
    )
    ...
    final = _event(
        ...
        "certify",
        final_status,
        "Trust Passport issued from replayed controls, verified provenance, human safety gates, and proven runtime recovery." if final_status == "CERTIFIED" else "Certification blocked because at least one required evidence, recovery, provenance, or audit-integrity gate failed.",
        {
...
def agents() -> dict:
    return {
        ...
        "certification_invariant": "No security claim without executed evidence; no remediation claim without comparable replay; no unprovenanced memory; no tool trust without integrity evidence; no failed runtime recovery may receive a Trust Passport; no high-risk real-world action without human approval.",
    }
...
def stream_gauntlet(
    ...
    return StreamingResponse(
        _live_run(run_id, target, pace_ms, recovery_mode),
        media_type="text/event-stream",
        headers={
            "Cache-Control": "no-cache",
            "X-Accel-Buffering": "no",
            "X-TRUSTFORGE-Run-ID": run_id,
        },
    )
...
def stream_gauntlet_post(request: RunRequest):
    run_id = f"tfz-{uuid.uuid4().hex[:12]}"
    return StreamingResponse(
        _live_run(run_id, request.target, request.pace_ms, request.recovery_mode),
        media_type="text/event-stream",
        headers={
            "Cache-Control": "no-cache",
            "X-Accel-Buffering": "no",
